subroutine grid_setup
   call grid_setup_radial !The new radial efficient grid
!  call grid_setup_old !The old rectangular awfully slow grid
end subroutine grid_setup
!=========================================================================================

subroutine grid_setup_radial
!..Sets up the radial grid with Becke construction for polyatomic systems
!..Writes basis functions on the grid points
!..Global
   use global; use grid_params; use functional_m; use basis_set, only:expo_bas_min
   implicit none

!..Local
   integer :: Idims(32)
   real(dp) :: rsb(105), rsb1, rsb3, rsbi, rsbj, x_be, u_be
   real(dp), allocatable :: rrad(:), wrad(:), vrad(:), a_be(:,:)
   real(dp), allocatable :: x_tr(:), y_tr(:), z_tr(:), w_tr(:), v_tr(:)
   real(dp) :: rgd, xat, yat, zat, wrd3, vrd3, w_at, xx, yy, zz
   real(dp), external :: at_weight, at_weight_old, f_bas, f_bas_pot 
   integer :: i, iat, jat, iele, jele, ilow, irad, iang, ig, igr, ngr, ibs

!..Dimensions for Lebedev angular grids
   Idims = (/ 6, 14, 26, 38,  50, 74,  86, 110, 146, 170, 194, 230, 266,  302,&
              350, 434, 590, 770, 974, 1202, 1454, 1730, 2030, 2354, 2702, 3074,&
              3470, 3890, 4334, 4802, 5294, 5810 /)

!..Slater Bragg Radius for elements in Angstom
   rsb = (/ 0.529177D+00,0.31D+00,1.45D+00,1.05D+00,0.85D+00, &
            0.70D+00, 0.65D+00, 0.60D+00, 0.50D+00, 0.38D+00, &
            1.80D+00, 1.50D+00, 1.25D+00, 1.10D+00, 1.00D+00, &
            1.00D+00, 1.00D+00, 0.71D+00, 2.20D+00, 1.80D+00, &
   ...
   rsb = rsb/0.529177d0 ! convert to Bohr

!..Cutoff radius for grid around atoms based on how diffuse basis set is
!..Maybe this should be atom dependent later
   rsb_cut_off= sqrt(abs(log(sqrt(tol_wg))/expo_bas_min))

   if( .not. allocated(rrad) ) allocate(rrad(nrad), wrad(nrad), vrad(nrad))

!..nang might not be one of the predifined values (Idims). Choose closest largest Idims 
   ilow = 0
   do i=1,32
      if( Idims(i) < nang ) ilow = i
   enddo
   nang=Idims(ilow+1)
   if(.not. allocated(xang)) allocate (xang(nang), yang(nang), zang(nang), wang(nang))

!..Create the a_ij matrix for the appropriate definition of Voronoi polyhedra
   if(.not. allocated(a_be)) allocate( a_be(natom, natom) )
   do iat=1, natom
      iele=min( Int(charge(iat)+0.1d0), 105)
      rsbi=rsb(iele)
      do jat=1,iat-1
         jele=min( Int(charge(jat)+0.1d0), 105)
         rsbj=rsb(jele)
         x_be=rsbi/rsbj
         u_be=(x_be-1.d0)/(x_be+1.d0)
         a_be(iat,jat) = u_be/(u_be*u_be-1.d0)
         a_be(iat,jat) = max( min(a_be(iat,jat), 0.5d0), -0.5d0)
         a_be(jat,iat) = -a_be(iat,jat)
      enddo
   enddo

!..Temporary grid allocation x_tr, y_tr, z_tr, w_tr
   ngr = natom*nrad*nang
   if(.not. allocated(x_tr)) allocate (x_tr(ngr), y_tr(ngr), z_tr(ngr), w_tr(ngr), v_tr(ngr))

!..Create generic radial grid (to be multiplied with rsb for each different atom)
   call generic_radial_grid(nrad, rrad, wrad, vrad)

!..Create generic radial grid xang**2+yang**2+zang**2 = 1
   call generic_angular_grid(nang, xang, yang, zang, wang)
   wang=4.d0*pi*wang !To integrate to 4 pi solid angle for sphere

!..Set grids on the atoms
   ig=1; ngrid=0
   do iat = 1, natom 
      xat=xcoo(iat); yat=ycoo(iat); zat=zcoo(iat)
      iele = min( Int(charge(iat)+0.1d0), 105)
      rsb1=rsb(iele); rsb3=rsb1*rsb1*rsb1
      do irad=1,nrad
         rgd = rsb1*rrad(irad); wrd3= rsb3*wrad(irad); vrd3=rsb3*vrad(irad)
         do iang=1,nang
            x_tr(ig) = xat + rgd*xang(iang)
            y_tr(ig) = yat + rgd*yang(iang)
            z_tr(ig) = zat + rgd*zang(iang)
            w_at =  at_weight(iat, x_tr(ig), y_tr(ig), z_tr(ig), a_be)
!           w_at =  at_weight_old(iat, x_tr(ig), y_tr(ig), z_tr(ig))
            w_tr(ig) = wang(iang)*wrd3*w_at
            v_tr(ig) = wang(iang)*vrd3*w_at
            if(w_tr(ig) > tol_wg) ngrid=ngrid+1 !Re-Define Grid dimension
            ig=ig+1
         enddo !iang=1,nang
      enddo !irad=1,nrad
   enddo !iat = 1, natom
 
   deallocate(rrad, wrad )

!..Allocate grid with dimension ngrid
   allocate( x_grid(ngrid), y_grid(ngrid), z_grid(ngrid), &
             w_grid(ngrid), v_grid(ngrid), bas_f_grid(ngrid, lnbasis) )

!..Prunning ... removing grid points with small weight
   igr=0
   do ig=1,ngr
      if(w_tr(ig) > tol_wg) then
         igr=igr+1 
         xx=x_tr(ig); yy=y_tr(ig); zz=z_tr(ig)
         x_grid(igr) = xx; y_grid(igr) = yy; z_grid(igr) = zz
         w_grid(igr) = w_tr(ig); v_grid(igr) = v_tr(ig)
         do ibs=1,nbasis
            bas_f_grid(igr,ibs) = f_bas(ibs, xx, yy, zz)
         enddo
      endif
   enddo

   deallocate ( x_tr, y_tr, z_tr, w_tr, v_tr)

   print*,'       Grid has been generated!  Size: ',ngrid, ' Points'
   grid_stat=.true.

end subroutine grid_setup_radial
!===============================================================================
function at_weight(iat, x, y, z, a_be)
!..For polyatomic systems Becke recipe
!..Global
...
!..Arguments
   integer :: iat
   real(dp), intent(IN) :: x, y, z,  a_be(natom,natom)

!..Local
   real(dp) :: P(natom), x_A, y_A, z_A, x_B, y_B, z_B
   real(dp) :: r_A, r_B, ss, xmu_ab
   integer :: i,j, ifu, int_fuz=4

   P = 1.d0
   do i=1, natom-1
      x_A=x-xcoo(i); y_A=y-ycoo(i); z_A=z-zcoo(i)
      r_A=sqrt(x_A*x_A + y_A*y_A + z_A*z_A)
      do j=i+1, natom
         x_B=x-xcoo(j); y_B=y-ycoo(j); z_B=z-zcoo(j)
         r_B=sqrt(x_B*x_B + y_B*y_B + z_B*z_B)
         xmu_ab=(r_A-r_B) / R_AB(i,j)
         xmu_ab=xmu_ab+a_be(i,j)*(1.d0-xmu_ab*xmu_ab)
         do ifu=1,int_fuz    
            xmu_ab=(1.5d0-0.5d0*xmu_ab*xmu_ab)*xmu_ab
         enddo
         xmu_ab=0.5d0*xmu_ab 
         P(i) = P(i) * (0.5  - xmu_ab) 
         P(j) = P(j) * (0.5  + xmu_ab)
...
end function at_weight
!===============================================================================

function at_weight_old(iat, x, y, z)
!..Global
   use global; use grid_params, only:Del_fuz; implicit none

!..The function
   real(dp) :: at_weight_old
...
!..Local
   real(dp) :: P, Pat, x_A, y_A, z_A, QAB, ss
   integer :: i,j
   ...
   do i=1, natom
      ...
      do j=1, natom
         if(j /= i) then
            QAB = 0.5d0*R_AB(i,j) - (x_A*x_AB(i,j)+y_A*y_AB(i,j)+z_A*z_AB(i,j))
            P = P*0.5d0*(1.d0+tanh (QAB/(Del_fuz*R_AB(i,j))))
         endif
      enddo
      if(i == iat) Pat=P
      ss=ss+P
   ...
   at_weight_old= Pat  / ss

end function at_weight_old
!=================================================================================

subroutine generic_radial_grid(nrad, rradg, wradg, vradg)
!..Global      
   use global; implicit none
   integer :: irad, nrad, nrad_max
   real(dp) :: rradg(nrad), wradg(nrad), vradg(nrad)
   real(dp) :: xirad, xdd, xd2, rrr, rr5, xnradmx
   real(dp) :: rrplus(nrad), rrminus(nrad)

!..Radial Grid from PMW Gill at al CPL 197, 499 (1992)
!..using Euler Mac Laurin quadrature defined in 
!..N.C. Handy, SF Boys, Theoret. Chim. Acta 31, 195 (1973)
!..C. W. Murray, N. C. Handy, G. J. Laming, Mol. Phys. 78, 997 (1993).
!  nrad: number of radial points
!  rradg(nrad): array with radial points
!  wradg(nrad): array with radial weights
   nrad_max=nrad+1
   rrminus(1) = 0.d0
   do irad=1,nrad
      xdd=nrad_max - irad; xirad=irad; xnradmx=nrad_max
      rrr=xirad/xdd
      rradg(irad) = rrr*rrr; xd2=xdd*xdd
      if(irad > 1) then
         rrminus(irad) = 0.5d0*(rradg(irad)+rradg(irad-1))
         rrplus(irad-1) = rrminus(irad)
      endif
!     if (rradg(irad) > rsb_cut_off) exit
      rr5=rrr*rrr*rrr*rrr*rrr
      wradg(irad) = 2.d0*xnradmx*rr5/xd2
!     if (rradg(irad) > rsb_cut_off) wradg(irad) =0.d0
   enddo
   rrplus(nrad) = 2.d0*rradg(nrad)-rrminus(nrad)
   do irad=1,nrad
      vradg(irad) = (rrplus(irad)**3-rrminus(irad)**3)/3.d0
   enddo
   Print*,'Number  of  radial points  (per atom): ', nrad
end subroutine generic_radial_grid
!=================================================================================

subroutine generic_angular_grid(nang, x, y, z, w)
   use global; implicit none
   integer :: nang,n
   real(dp) :: x(nang), y(nang), z(nang), w(nang)
   ...
   select case (nang)
      ...
      case default
         stop 'generic_angular_grid: No such case for nang'
   end select

   print*,'Number of Angular points (per radius): ', N

end subroutine generic_angular_grid

!==========================================================================================
!...The next Fortran 77 routines are taken from :
!... /* http://www.ccl.net/ */
...
!VW
       ...
       END
!===================================================================================
! End of Fortrann 77 routines copied from 
! /* http://www.ccl.net/ */

subroutine grid_setup_old
!..Global
   use global; use grid_params; use functional_m; implicit none

!..Local
   integer :: iatom, i,j,k,ijk, ig, ibs
   real(dp) :: xmin, xmax, ymin, ymax, zmin, zmax, dx3, xt, yt, zt
   real(dp) :: Del_x, Del_y, Del_z, exce_x, exce_y, exce_z, corr_0
   real(dp) :: dx3_2, dx3_4, dx3_8, dx3_16, dx3_32, dx3_64, fu_val
   real(dp), allocatable :: w_grid_tmp(:,:,:)
   real(dp), external :: f_bas, f_bas_pot
   logical, allocatable :: yn_f_grid(:,:,:)
   logical ::  yn1, yn2
...
!..Find the coordinate extremes
   ...
   zmin = 1d20; zmax=-1d20
   do iatom=1,natom
      if(xcoo(iatom) < xmin ) xmin = xcoo(iatom)
      if(xcoo(iatom) > xmax ) xmax = xcoo(iatom) 
      if(ycoo(iatom) < ymin ) ymin = ycoo(iatom)
      if(ycoo(iatom) > ymax ) ymax = ycoo(iatom) 
      if(zcoo(iatom) < zmin ) zmin = zcoo(iatom)
      if(zcoo(iatom) > zmax ) zmax = zcoo(iatom) 
   enddo
   
!..Extend Cartesian Grid by coo_extend (parameter above)
   if(coo_extend < zero ) stop 'grid_int: coo_extend infinitesimally small'
   xmin = xmin - coo_extend
   ymin = ymin - coo_extend
   zmin = zmin - coo_extend
   xmax = xmax + coo_extend
   ymax = ymax + coo_extend
   zmax = zmax + coo_extend
...
!..Match boundaries
   n_x = 0.0001d0 + Del_x / dx
   n_y = 0.0001d0 + Del_y / dy
   n_z = 0.0001d0 + Del_z / dz

   exce_x = 0.5d0*(n_x*dx - Del_x)
   exce_y = 0.5d0*(n_y*dy - Del_y)
   exce_z = 0.5d0*(n_z*dz - Del_z)
   ...
   n_x=n_x+1; n_y=n_y+1; n_z=n_z+1

!..We want even number for Simpson quadrature
   if(mod(n_x, 2) == 0 ) then
     n_x = n_x + 1
     xmin = xmin - 0.5d0*dx
     xmax = xmax + 0.5d0*dx
...
!..Mesh sorted, lets allocate w_grid_tmp, yn_f_grid
   allocate( w_grid_tmp(n_x, n_y, n_z), &
             yn_f_grid(n_x,n_y,n_z)) 


!..Set up the weights
   print*,'Grid Boundaries (a.u.):'
   print*,'X: ',xmin,' - ',xmax
   print*,'Y: ',ymin,' - ',ymax
   print*,'Z: ',zmin,' - ',zmax
   print*,'Grid area volume:',(xmax-xmin)*(ymax-ymin)*(zmax-zmin)
   dx3 = dx*dy*dz/27.d0
   dx3_2 = 2.d0*dx3; dx3_4 = 4.d0*dx3
   dx3_8 = 8.d0*dx3; dx3_16 = 16.d0*dx3
   dx3_32 = 32.d0*dx3; dx3_64 = 64.d0*dx3

   w_grid_tmp=0.d0
!..At cube corners
   w_grid_tmp(1,1,1)= dx3; w_grid_tmp(n_x,1,1)= dx3 
   w_grid_tmp(1,n_y,1)= dx3; w_grid_tmp(1,1,n_z)= dx3 
   w_grid_tmp(1,n_y,n_z)= dx3; w_grid_tmp(n_x,1,n_z)= dx3 
   w_grid_tmp(n_x,n_y,1)= dx3; w_grid_tmp(n_x,n_y,n_z)= dx3 

!..In cube edges
   do k=2,n_z-1
      if (mod(k,2) == 0) then
         w_grid_tmp(1,1,k) = dx3_4
         w_grid_tmp(1,n_y,k) = dx3_4
         w_grid_tmp(n_x,1,k) = dx3_4
         w_grid_tmp(n_x,n_y,k) = dx3_4
      else
         w_grid_tmp(1,1,k) = dx3_2
         w_grid_tmp(1,n_y,k) = dx3_2
         w_grid_tmp(n_x,1,k) = dx3_2
         w_grid_tmp(n_x,n_y,k) = dx3_2
      endif
   enddo

   do j=2,n_y-1
      if (mod(j,2) == 0) then
         w_grid_tmp(1,j,1) = dx3_4
         w_grid_tmp(1,j,n_z) = dx3_4
         w_grid_tmp(n_x,j,1) = dx3_4
         w_grid_tmp(n_x,j,n_z) = dx3_4
      else
         w_grid_tmp(1,j,1) = dx3_2
         w_grid_tmp(1,j,n_z) = dx3_2
         w_grid_tmp(n_x,j,1) = dx3_2
         w_grid_tmp(n_x,j,n_z) = dx3_2
      endif
   enddo

   do i=2,n_x-1
      if (mod(i,2) == 0) then
         w_grid_tmp(i,1,1) = dx3_4
         w_grid_tmp(i,1,n_z) = dx3_4
         w_grid_tmp(i,n_y,1) = dx3_4
         w_grid_tmp(i,n_y,n_z) = dx3_4
      else
         w_grid_tmp(i,1,1) = dx3_2
         w_grid_tmp(i,1,n_z) = dx3_2
         w_grid_tmp(i,n_y,1) = dx3_2
         w_grid_tmp(i,n_y,n_z) = dx3_2
      endif
   enddo
...
!..On cube faces
   do j=2,n_y-1
      do k=2,n_z-1
         ...
         if( mod(j, 2) == 0 .and. mod(k, 2) == 0) then
            w_grid_tmp(1,j,k) = dx3_16
            w_grid_tmp(n_x,j,k) = dx3_16
         elseif( mod(ijk, 2) == 0) then
            w_grid_tmp(1,j,k) = dx3_8
            w_grid_tmp(n_x,j,k) = dx3_8
         else
            w_grid_tmp(1,j,k) = dx3_4
            w_grid_tmp(n_x,j,k) = dx3_4
         endif
      enddo
   enddo
   ...
   do i=2,n_x-1
      do k=2,n_z-1
         ijk=i*k
         if( mod(i, 2) == 0 .and. mod(k,2) == 0) then
            w_grid_tmp(i,1,k) = dx3_16
            w_grid_tmp(i,n_y,k) = dx3_16
         elseif( mod(ijk, 2) == 0) then
            w_grid_tmp(i,1,k) = dx3_8
            w_grid_tmp(i,n_y,k) = dx3_8
         else
            w_grid_tmp(i,1,k) = dx3_4
            w_grid_tmp(i,n_y,k) = dx3_4
         endif
      enddo
   enddo
   ...
   do i=2,n_x-1
      do j=2,n_y-1
         ...
         if( mod(i, 2) == 0 .and. mod(j,2) == 0) then
            w_grid_tmp(i,j,1) = dx3_16
            w_grid_tmp(i,j,n_z) = dx3_16
         elseif( mod(ijk, 2) == 0) then
            w_grid_tmp(i,j,1) = dx3_8
            w_grid_tmp(i,j,n_z) = dx3_8
         else
            w_grid_tmp(i,j,1) = dx3_4
            w_grid_tmp(i,j,n_z) = dx3_4
         endif
      enddo
   enddo
      ...
      do j = 2, n_y-1
         do k = 2, n_z-1
            ...
            if( mod(i,2)==0 .and. mod(j,2)==0 .and. mod(k,2)==0) then 
               w_grid_tmp(i,j,k) = dx3_64
            elseif( (mod(i,2)==0 .and. mod(j,2)==0) .or.& 
                    (mod(i,2)==0 .and. mod(k,2)==0) .or.&
                    (mod(k,2)==0 .and. mod(j,2)==0) ) then
               w_grid_tmp(i,j,k) = dx3_32
            elseif( mod(ijk,2) == 0) then
               w_grid_tmp(i,j,k) = dx3_16
            else
               w_grid_tmp(i,j,k) = dx3_8
            endif
         enddo
      enddo
   enddo

!..And the mesh points... memory sacrifice for speed
!  ngrid=0
!  xt=xmin
!  do i = 1, n_x
!     yt=ymin
...
!        zt=zmin
!        do k = 1, n_z
!           yn_f_grid(i,j,k)=.false.
!           do ibs=1,nbasis; do jbs=1,ibs; do kbs=1,nbasis_pot
!              fu_val = abs(f_bas(ibs,xt,yt,zt)*f_bas(jbs,xt,yt,zt)&
!                           * f_bas_pot(kbs,xt,yt,zt))
!              if(fu_val > small_f ) then
!                 yn_f_grid(i,j,k)=.true.
!                 ngrid=ngrid+1
!                 exit
!              endif
!           enddo; if(fu_val > small_f ) exit; enddo; if(fu_val > small_f ) exit; enddo
!           zt = zt + dz
!        enddo
!        yt=yt + dy
!     enddo
   ...
   do i = 1, n_x
      ...
      do j = 1, n_y
         ...
         do k = 1, n_z
            ...
            enddo
!           do ibs=1,nbasis_pot
!              fu_val = abs(f_bas_pot(ibs,xt,yt,zt))
!              if(fu_val > small_f ) then
!                 yn2= .true.
!                 exit
!              endif
!           enddo
   ...
   print*,'Number of active grid points:', ngrid
 
   if(ngrid < 1) stop 'grid_setup: ngrid < 1'

   allocate (x_grid(ngrid), y_grid(ngrid), z_grid(ngrid), &
             w_grid(ngrid), bas_f_grid(ngrid, lnbasis) )
   ...
   do i = 1, n_x
      ...
      do j = 1, n_y
         ...
         do k = 1, n_z
            ...
            if( yn_f_grid(i,j,k) ) then
               ig=ig+1
               x_grid(ig) = xt
               y_grid(ig) = yt
               z_grid(ig) = zt
               w_grid(ig) = w_grid_tmp(i,j,k)
               do ibs=1,nbasis
                  bas_f_grid(ig,ibs) = f_bas(ibs,xt,yt,zt)
               enddo
            endif !yn_f_grid(i,j,k)
...
!..|r-r^\prime| on double drid

   deallocate(w_grid_tmp, yn_f_grid) 

   grid_stat=.true.
   print*,'Grid is set'
end subroutine grid_setup_old

subroutine orbs_on_grid(Norb)
!..Global
   use global; use grid_params; use orbocc; implicit none
...
!..Local
   ...
   if(.not.grid_stat) call grid_setup

   if (.not.allocated(vec_nat_grid)) allocate ( vec_nat_grid(ngrid,lnnatorb) )
   if (.not.allocated(orbgridsq)) allocate ( orbgridsq(ngrid,lnnatorb) )

!$OMP PARALLEL DEFAULT(SHARED) &
!$OMP PRIVATE(ig,ia,ibs)
!$OMP DO
   do ig=1,ngrid
      do ia=1,Norb
         vec_nat_grid(ig,ia)=0.d0; orbgridsq(ig,ia)=0.d0
         do ibs=1,nbasis
            vec_nat_grid(ig,ia)=vec_nat_grid(ig,ia)+vecnat(ibs,ia)*bas_f_grid(ig,ibs)
         enddo
         orbgridsq(ig,ia)= vec_nat_grid(ig,ia)*vec_nat_grid(ig,ia)
      enddo
   enddo
!$OMP END DO
!$OMP END PARALLEL
